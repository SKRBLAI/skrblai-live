...
export default function HomePage() {
  ...
  // --- Agent Filtering/Demo Modal State ---
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [demoAgent, setDemoAgent] = useState<any>(null);
  const allCategories = Array.from(new Set(agentDashboardList.flatMap(agent => agent.agentCategory || [])));
  const filteredAgents = selectedCategory === 'all'
    ? agentDashboardList
    : agentDashboardList.filter(agent => (agent.agentCategory || []).includes(selectedCategory));
  ...
  return (
    <main className="min-h-screen relative text-white bg-[#0d1117] pt-16 overflow-hidden">
      ...
      <div className="relative z-10 pt-8 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        {/* Agent Highlights Section with Animated Cosmic Background, Filter & Demo Modal */}
        <div className="relative mt-16">
          ...
          <div className="absolute inset-0 z-0 pointer-events-none">
            ...
            <div className="absolute inset-0 animate-cosmic-gradient bg-[radial-gradient(ellipse_at_60%_40%,rgba(56,189,248,0.16)_0%,rgba(244,114,182,0.09)_60%,transparent_100%)]" style={{mixBlendMode:'screen'}} />
          </div>
          ...
          {allCategories.map(cat => {
            // Emoji icon mapping for categories (customize as needed)
            const icon =
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 justify-center">
            {filteredAgents.map(agent => {
              // --- Badge logic (mock: Pro for roleRequired, Beta for isBeta, New for isNew) ---
              let badge = null;
              if (agent.roleRequired === 'pro') badge = 'Pro';
              else if (agent.isBeta) badge = 'Beta';
              else if (agent.isNew) badge = 'New';
              // For demo, show Pro for paymentManagerAgent, Beta for percySyncAgent, New for videoContentAgent
              ...
              return (
                ...
                >
                  ...
                  >
                    <Image
                      src={`/images/agents/agents/${agent.imageSlug || agent.id}.png`}
                      alt={agent.name}
                      width={96}
                      height={96}
        ...
        {demoAgent && (
          ...
          >
            <motion.div
              className="relative bg-gradient-to-br from-electric-blue/70 via-fuchsia-500/40 to-teal-400/70 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full flex flex-col items-center animate__animated animate__fadeInDown border-4 border-fuchsia-400/80 animate-border-glow"
              ...
              tabIndex={0}
              onKeyDown={e => {
                if (e.key === 'Escape') setDemoAgent(null);
                // Trap focus inside modal
                if (e.key === 'Tab') {
                  const focusable = Array.from((e.currentTarget as HTMLElement).querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')) as HTMLElement[];
            ...
            >
              ...
              <div className="mb-4 flex flex-col items-center">
                <Image
                  src={`/images/agents/agents/${demoAgent.imageSlug || demoAgent.id}.png`}
                  alt={demoAgent.name}
                  width={120}
                  height={120}
              ...
              {/* Percy gets the real chat, others get a placeholder sample */}
              {demoAgent.id === 'percy-agent' ? (
                <div className="w-full">
                  {/* If you have a dedicated PercyChat, swap below; fallback to PercyHero */}
                  <PercyChat />
                </div>
              ) : (
        ...
        </div> {/* End relative mt-16 (cosmic bg) */}
