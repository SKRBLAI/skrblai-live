-- ============================================================================
-- Migration: Fix All Supabase Linter Warnings (CORRECTED VERSION)
-- Date: 2025-10-24
-- Purpose: Address all 54 warnings with correct column names
-- ============================================================================
...
-- 6. subscription_conversion_funnel - User policy (uses 'userid')
...
-- 7-9. agent_launches - Drop old policies (will create consolidated ones)
...
-- 10. percy_intelligence_events - User policy (uses 'userid')
...
-- 11. percy_contexts - User policy (uses 'userid')
...
-- 12. agent_access_logs - User policy (uses 'userid')
...
-- 13-14. n8n_executions - Drop old policies
...
-- 15-16. system_health_logs - Drop old policies
...
-- agent_launches - Consolidate (uses 'user_id')
...
-- founder_codes - Consolidate SELECT
...
-- founder_memberships - Consolidate (uses 'user_id')
...
-- n8n_executions - Consolidate (uses 'user_id')
...
-- system_health_logs - Consolidate
...
-- 2. update_percy_context_timestamp (drop trigger first)
DROP TRIGGER IF EXISTS trigger_percy_context_updated_at ON public.percy_contexts;
DROP FUNCTION IF EXISTS public.update_percy_context_timestamp() CASCADE;
...
BEFORE UPDATE ON public.percy_contexts
FOR EACH ROW
EXECUTE FUNCTION public.update_percy_context_timestamp();
...
CREATE INDEX IF NOT EXISTS idx_percy_intelligence_events_userid_rls ON public.percy_intelligence_events(userid);
CREATE INDEX IF NOT EXISTS idx_percy_contexts_userid_rls ON public.percy_contexts(userid);
CREATE INDEX IF NOT EXISTS idx_agent_access_logs_userid_rls ON public.agent_access_logs(userid);
...
SELECT '✅ Performance and Security Migration v2 (2025-10-24) completed' AS status;
