...
export default function PricingCard({ 
  ...
  const handlePurchase = () => {
    ...
    // For plans with configured Stripe price IDs, we'll use the CheckoutButton
    // The button will handle the checkout process directly
  };
  ...
  return (
    ...
    >
      ...
      <CardShell className={`p-8 h-full flex flex-col transition-all duration-500 ${
        ...
        {/* CTA Button */}
        {plan.monthlyPrice === 0 || !plan.stripePriceIds ? (
          <CosmicButton
          ...
          <CheckoutButton
            label={`${ctaText} ${plan.icon}`}
            priceId={billingPeriod === 'monthly' ? plan.stripePriceIds.monthly : plan.stripePriceIds.annual}
            mode="subscription"
            className={`w-full font-bold text-sm transition-all duration-200 h-11 rounded-lg ${
              plan.isEnterprise 
                ? 'bg-gradient-to-br from-violet-700/30 via-purple-800/30 to-indigo-800/30 backdrop-blur-lg text-cyan-300 border border-teal-400/30 hover:bg-teal-500/20' 
                : plan.isPopular 
                ? 'btn-solid-grad animate-pulse' 
                : 'bg-gradient-to-br from-gray-700/50 to-gray-800/50 backdrop-blur-lg text-cyan-300 border border-teal-400/30 hover:bg-teal-500/20'
            }`}
          />
        )}
